...
create table user_connecions (
  user_id                        bigint not null,
  connection_id                  bigint not null,
  constraint pk_user_connecions primary key (user_id, connection_id))
;
...
alter table user_connecions add constraint fk_user_connecions_user_01 foreign key (user_id) references user (id) on delete restrict on update restrict;

alter table user_connecions add constraint fk_user_connecions_user_02 foreign key (connection_id) references user (id) on delete restrict on update restrict;
